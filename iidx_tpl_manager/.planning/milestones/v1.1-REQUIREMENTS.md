# Requirements Archive: v1.1 Tournament Rule Refinements
...
- [x] **CONF-02**: Application loads `teams.json`, `team_schedule.json`, and `individual_schedule.json` from a `data/` subdirectory
- [x] **CONF-03**: Application validates loaded JSON configs against expected schemas and reports human-readable errors
- [ ] **CONF-04**: Operator can assign each player to one of 4 cabinet numbers on a round-prep page before each round
- [x] **CONF-05**: Runtime state (loaded configs and file paths) is persisted to disk and auto-restored on startup
