#!/usr/bin/env python3
"""OBS overlay 链路 smoke test（手动验证用，非 pytest）。

真实启动：overlay relay(8082) + 导播台 app(5003) + 假记分板(8080/8081)，
通过 HTTP API 走一遍团队赛单回合流程，断言：
- 8080 收到 init 与 score；
- overlay 客户端(8082)收到 stage/activate 与 round_result，并确认前两者。

用法（iidx_director/ 目录下）：
    python overlay/smoke_test.py
"""
    ...
    async with websockets.connect("ws://localhost:8082") as ws:
        try:
            while True:
                ...
                received["overlay"].append(message)
                if message.get("cmd") in {"stage", "activate"} and message.get("request_id"):
                    await ws.send(json.dumps({
                        "cmd": "ack", "request_id": message["request_id"], "ok": True,
                    }))
...
async def main() -> int:
    ...
    relay = subprocess.Popen([PYTHON, str(ROOT / "overlay" / "server.py")])
    app_proc = subprocess.Popen(
        [PYTHON, "-m", "src.app", "--no-autostart"], cwd=ROOT,
    ...
    try:
        ...
        begin = await post("/api/round/begin", {})
        assert begin["success"]
        assert (await post("/api/scene/pending/confirm", {"id": begin["pending"]["id"]}))["success"]
        await asyncio.sleep(1)
        assert (await post("/api/round/force_review", {"scores": {"选手L1": 2000, "选手R1": 1500}}))["success"]
        confirm = await post("/api/round/confirm", {"scores": {"选手L1": 2000, "选手R1": 1500}})
        assert confirm["success"]
        assert (await post("/api/scene/pending/confirm", {"id": confirm["pending"]["id"]}))["success"]
        await asyncio.sleep(1)
        ...
        ok &= "stage" in cmds_overlay and "activate" in cmds_overlay and "round_result" in cmds_overlay
        score = next(m for m in received["8080"] if m.get("cmd") == "score")
        ok &= score["data"] == {"round": 1, "leftScore": 1, "rightScore": 0}
        staged = next(m for m in received["overlay"] if m.get("cmd") == "stage")
        ok &= staged["scene"] == "SP_BPL" and staged["snapshot"]["template"] == "sp_bpl"
