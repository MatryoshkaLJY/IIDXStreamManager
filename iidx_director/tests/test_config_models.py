...
@pytest.mark.parametrize("model_factory", [TeamMatchConfig, KnockoutConfig])
def test_play_type_accepts_sp_and_dp_and_rejects_other_values(model_factory):
    raw = _team_match_dict() if model_factory is TeamMatchConfig else _knockout_dict()
    raw["playType"] = "DP"
    assert model_factory.model_validate(raw).play_type == "DP"
    raw["playType"] = "INVALID"
    with pytest.raises(Exception, match="playType|SP|DP"):
        model_factory.model_validate(raw)
